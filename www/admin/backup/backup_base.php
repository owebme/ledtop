...
if (!$auth || (isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] == 'reload')) {
	setcookie("sxd");
	echo tpl_page(tpl_auth($error ? tpl_error($error) : ''), "<SCRIPT>if (jsEnabled) {document.write('<INPUT class='button_backup' TYPE=submit VALUE=Применить>');}</SCRIPT>");
...
class dumper {
	...
	function backup() {
		...
		$buttons = "<A ID=save HREF='' STYLE='display: none;'>Скачать файл</A> &nbsp; <INPUT class='button_backup' ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
		...
		echo "<SCRIPT>with (document.getElementById('save')) {style.display = 'none'; innerHTML = 'Скачать файл ({$filesize})'; href = '" . URL . $this->filename . "'; }document.getElementById('back').disabled = 0;</SCRIPT>";
	...
	function restore(){
		if (!isset($_POST)) {$this->main();}
		set_error_handler("SXD_errorHandler");
		$buttons = "<INPUT class='button_backup' ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
	...
	function main(){
		...
		$buttons = "<INPUT class='button_backup' TYPE=submit VALUE=Применить><!--INPUT TYPE=button VALUE=Выход onClick=\"location.href = 'backup_base.php?reload'\"-->";
	...
	function fn_select($items, $selected){
		...
		foreach($items AS $key => $value){
			$select .= $key == $selected ? "<OPTION VALUE='{$key}' SELECTED>{$value}" : "<OPTION style='height:23px; font:13px Tahoma; padding-top:5px;' VALUE='{$key}'>{$value}";
		}
		return $select;
	}
...
input.button_backup {width:117px; height:29px; background:url(/admin/img/pages_button.png) no-repeat; border:none; cursor:pointer; padding-bottom:4px;}
-->
</STYLE>
</HEAD>

<BODY TEXT=#000000>
<h2 style="font:normal 18px Arial; color:#666; margin-bottom:17px; margin-top:0px;">Восстановление базы (MySQL)</h2>
<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0 ALIGN=LEFT style="margin-left:11px;">
<TR>
...
<TR>
<FORM NAME=skb METHOD=POST ACTION=backup_base.php>
...
<TR>
<TD STYLE='color: #CECECE; display:none;' ID=timer></TD>
...
Создание резервной копии БД&nbsp;</LEGEND>
...
Восстановление БД из резервной копии&nbsp;</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2 style="margin-top:7px;">
<TR>
<TD>База данных:</TD>
<TD><SELECT NAME=db_restore style="font:13px Tahoma;">
{$SK->vars['db_restore']}
</SELECT></TD>
</TR>
<TR>
<TD WIDTH=35%>Выберите бэкап:</TD>
<TD WIDTH=65%><SELECT NAME=file style="height:25px; font:13px Tahoma; padding-top:3px;">
{$SK->vars['files']}
</SELECT></TD>
</TR>
</TABLE>
