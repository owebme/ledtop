...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _filetype($filename) {
		if (strpos($filename, ".")) {
		...
		if ($filetype == "php" or $filetype == "cgi" or $filetype == "pl" or $filetype == "pm"){		
			return 1;
		}
	}	
	...
	protected function _extract($path, $arc) {
		...
		if ($this->quarantine) {
			...
			if (count($ls) == 1) {
				...
				if ($file_unlink){
					unlink(dirname($path)."/".$file_unlink);
					unlink($path);
				}
				...
				if ($objs = glob($result."/*")) {
				   foreach($objs as $obj) {
						if (!is_dir($obj)){
							preg_match_all("%".$result."\/(.+?)$%si", $obj, $res);
							$file = $res[1][0];
							if ($this->_filetype($file)){
								unlink($obj);
							}
