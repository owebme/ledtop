$(document).ready(function(){
		...
		steps: [
			{
			  ...
			  intro: 'Список страниц сайта',
			  position: 'top'
			},
			{
			  element: 'div.three_pages ul li a.del',
			  intro: 'Удалить страницу',
			  position: 'bottom'
			},
			{
			  element: 'div.three_pages ul li a.lamp',
			  intro: 'Сделать активным / не активным страницу для отображения на сайте',
			  position: 'bottom'
			},
			{
			  element: 'div.three_pages ul li span.move',
			  intro: 'Управление порядком сортировки страниц',
			  position: 'bottom'
			},	
			{
			  element: 'div.three_pages ul li a.name',
			  intro: 'Наведите на название страницы и вы увидите ее адрес, кликнув на нее вы перейдете к редактированию',
			  position: 'bottom'
			},			 
			{
	...
	$("a#help-tour").live('click', function(){
		if ($("div.three_pages ul").html() == ""){
			var el = $(this).parent();
			$(el).html('<div class="warning">'+$(el).html()+'<span>Добавьте хотя бы одну страницу</span></div>');
