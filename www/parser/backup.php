...
if (!empty($user) && isset($mypassword)) {
//	if (@mysql_connect(DBHOST, $_POST['login'], $_POST['pass'])){
	if (@mysql_connect(DBHOST, $user, $mypassword)){
//		setcookie("sxd", base64_encode("SKD101:{$_POST['login']}:{$_POST['pass']}"));
		setcookie("sxd", base64_encode("SKD101:{$user}:{$mypassword}"));
...
class dumper {
	...
	function backup() {
		...
		$this->SET['last_action']     = 0;
		$this->SET['last_db_backup']  = isset($_GET['db_backup']) ? $_GET['db_backup'] : '';
		$this->SET['tables_exclude']  = !empty($_GET['tables']) && $_GET['tables']{0} == '^' ? 1 : 0;
		$this->SET['tables']          = isset($_GET['tables']) ? $_GET['tables'] : '';
		$this->SET['comp_method']     = isset($_GET['comp_method']) ? intval($_GET['comp_method']) : 0;
		$this->SET['comp_level']      = isset($_GET['comp_level']) ? intval($_GET['comp_level']) : 0;
		$this->fn_save();
		...
		$func = $_GET['time'];
		$name = 'db-backup_' . date("Y-m-d_H-i") . '_'.$func;
...
//		if (GS) echo "<SCRIPT>document.getElementById('GS').src = 'http://sypex.net/gs.php?r={$this->tabs},{$this->records},{$this->size},{$this->comp},107';</SCRIPT>";
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><!--INPUT TYPE=button VALUE=Выход onClick=\"location.href = 'backup.php?reload'\"-->";
